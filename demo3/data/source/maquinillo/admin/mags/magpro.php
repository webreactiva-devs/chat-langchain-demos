...
elseif($_GET['action'] == 'addjour'){
	...
	elseif($_POST['btn_jour']){
		...
		$query="INSERT INTO cms_journals 
				 (title, description, text, path, url, journal_type, groupID, imageID) 
				VALUES ('$_POST[title]', '$_POST[description]', '$_POST[text]', '$_POST[path]', '$_POST[url]', '$_POST[journal_type]', '$_POST[groupID]', '$_POST[imageID]')";
...
elseif($_GET['action'] == 'editjour'){
	...
	if($_POST['btn_jour']){
		...
		$query="UPDATE cms_journals 
				SET title='$_POST[title]', description='$_POST[description]', text='$_POST[text]', path='$_POST[path]', url='$_POST[url]', journal_type='$_POST[journal_type]', groupID='$_POST[groupID]', imageID='$_POST[imageID]' 
...
elseif($_GET['action'] == 'deletejour'){
	...
	if(!$_POST['btn_deljour']){
		echo printMsg('¿Seguro que quiere borrar el magazine completo (y todas las entradas del mismo)?');
	...
	elseif($_POST['btn_deljour']){
		...
		if($result && $result2){
			echo printMsg('Magazine eliminado con éxito');
			$siteDB->free_result();		
		}
		else{
