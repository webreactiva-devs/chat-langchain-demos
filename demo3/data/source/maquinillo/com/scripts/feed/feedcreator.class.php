...
class FeedCreator {
	 ...
	 */
	var $syndicationURL, $image, $language, $copyright, $pubDate, $lastBuildDate, $editor, $editorEmail, $webmaster, $category, $docs, $ttl, $rating, $skipHours, $skipDays, $contentEnc;
...
class RSSCreator10 extends FeedCreator {
	...
	function createFeed() {     
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			if ($this->items[$i]->contentEnc!=""){
				$feed.= "        <content:encoded>\n<![CDATA[ ".escapeBoulesisData($this->items[$i]->contentEnc)."]]>\n</content:encoded>\n";
			}
			$feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "        ");
			$feed.= "    </item>\n";
		}
...
function escapeBoulesisData($string)
{
	$string = str_replace(']]>', ']]&gt;', $string);
	$string = ereg_replace("<exe (.+) />", '', $string);
	return $string;
}
